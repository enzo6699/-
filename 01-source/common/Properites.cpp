...
namespace tce
{
	...
	void Properties::load(const std::string& initFileName) throw (ConfigureFailure)
	{
		std::fstream initFile(initFileName);
		if (!initFile)
			throw ConfigureFailure(std::string("File ") + initFileName + " does not exist or pemission denied");
    ...
    float Properties::getFloat(const std::string &property, float defaultValue)
    {
        const_iterator key = find(property);
        return (key == end()) ? defaultValue : std::atof((*key).second.c_str());
    }
